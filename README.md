...
> 当前预发布版本：**v0.9.4-rc**（Windows 安装版 / 免安装 zip · Linux AppImage / deb）
...
| 页面 | 功能 |
| --- | --- |
| **总览** | 内核启停（大按钮）、运行状态/内核版本、系统代理开关、实时流量卡片（速率曲线+连接数+累计）、快速开始 |
...
| 层 | 工具 | 覆盖 |
| --- | --- | --- |
| 单元 | Vitest | 配置解析/订阅导入（含 URI 转换）/TUN 开关/档案管理（19 项） |
| 集成 | `core:e2e` | 进程驱动全链路：导入→启停→路由→流量累计→热重载（17 项） |
