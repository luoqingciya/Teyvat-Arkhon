...
- **订阅配额展示**：URL 订阅解析 `subscription-userinfo` 响应头，订阅列表直接展示已用/总流量进度条与到期倒计时，刷新订阅自动更新。
- **策略组操作**：策略组切换、单节点测速、**批量测速**（限量并发，逐行反馈进度）、按延迟排序。
- **延迟快照展示**：打开代理页即呈现各节点最近一次测速缓存（定制端点 `/delay/latest`，不再重复测速），切换页面/进页即时可见。
...
- **可视化分流规则编辑**：「分流规则」页表格化管理 `rules` 与 `rule-providers`（类型/目标/策略 + 上下移/删除/行级校验/命中数展示），支持远程与本地的 rule-provider 管理预览、内置分流预设（直连/广告/流媒体/AI/中国直连）一键套用，以及域名/IP 的规则命中逐步调试。
- **DNS 分流联动**：「DNS 分流」页可视化配置 nameserver / fallback / nameserver-policy / fake-ip，域名级策略图形化（如 `.cn → 223.5.5.5`），内置三套 DNS 分流预设，保存即热重载，为规则分流补充"解析侧"分流。
...
| **代理** | 节点/规则双视图：策略组切换（chips）、节点表格（类型/延迟）、单点测速、批量测速（带进度）、按延迟排序、选中高亮；延迟快照即时展示最近测速结果；规则视图（类型/目标/目标组/命中，搜索+按命中排序） |
| **订阅** | URL 导入、文本导入（textarea）、档案列表（使用中/刷新/删除/切换）、订阅配额展示（已用/总量进度条 + 到期倒计时） |
| **分流规则** | 4 Tab：规则编辑器（表格化增删移/行级校验/命中数）、规则集管理（http/file + 在线预览）、内置分流预设一键套用、规则命中逐步调试 |
| **DNS 分流** | 3 Tab：解析通道（enable/ipv6/增强模式/fake-ip 池/nameserver/fallback 多行）、域名策略（nameserver-policy 图形化）、内置 DNS 分流预设；保存即热重载 |
| **连接** | 活跃连接表格（1s 刷新）、单条关闭、全部断开、累计流量总览 |
| **配置** | CodeMirror YAML 编辑、保存即校验 + 热重载、校验失败回显错误 |
| **设置** | TUN 开关、开机自启、订阅自动更新与排除关键词、测速参数、网络自检、UWP 回环、主题（亮/暗）、语言（zh/en）、数据目录与便携、系统服务（安装/卸载/状态）、关于（版本/许可证） |
...
pnpm dev                            # 开发模式
pnpm core:e2e                       # E2E 全链路回归（含延迟快照断言，需已放置内核二进制）
...
| 层 | 工具 | 覆盖 |
| --- | --- | --- |
| 单元 | Vitest | 配置解析/订阅导入（含 URI 转换与全参数映射）/TUN 开关/档案管理/模式持久化/订阅配额解析（24+ 项） |
| 集成 | `core:e2e` | 进程驱动全链路：导入→启停→路由→流量累计→热重载→模式切换→规则数据面→延迟快照（内核 `/delay/latest`） |
| UI | Playwright (_electron) | 启动渲染、导航、订阅/设置页可达（含开机自启开关） |
