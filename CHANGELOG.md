...
### Changed
- **发布可靠性**：release 流水线新增「内核全链路 E2E 闸门」（订阅导入→启动→路由→流量→热重载）与「打包产物启动冒烟」（应用进程存活检测），杜绝"本机通过、打包即崩"
- **完整性校验**：`download-core.mjs` 新增内核/geo 的 SHA256 校验清单 `deps.sha256.json`；登记过的版本+平台做强校验，未登记则警告提示
